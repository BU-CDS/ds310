...
## How to Evaluate a SELECT command
...
* The order of operations to evaluate a SELECT command is:
    1. evaluate any subqueries in the FROM clause
    2. form the Cartesian product of the tables in the FROM clause: `table1 x table2 x …`
    3. if there is an OUTER JOIN, apply its join condition and add extra tuples as needed
    4. apply the remaining clauses in the following order
        - WHERE (including any subqueries)
        - GROUP BY
        - HAVING
        - SELECT
        - ORDER BY
...
Try these before the next lecture! (We'll go over them in class.)
